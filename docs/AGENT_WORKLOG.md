...
| 2026-08-28 | 서버 전용 OpenAI 문장화 및 결정론적 fallback | core-logic (Luna Max) | 1.2.0 | REVISION | `services/openai_service.py`, `api/index.py`, 관련 오프라인 테스트 | 수정 1차에서 3/4 결함 해결; 재리뷰에서 `진단 추가 취소` 의미 부정 우회 1건 잔존 | 2 | Task 6; 허용 문장 골격 외 표현은 fallback하는 수정 라운드 2/5 진행 |
