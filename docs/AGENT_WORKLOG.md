...
| 2026-08-28 | 가상 원본 기록 조회·입력 샌드박스 | frontend (Luna Max) + supervisor | 1.2.0 | PASSED | 세션 기록 경계, 차트 장부형 드로어, 구조화 입력, 성공 시 비교 결과 교체, `0.6.0` 문서 | 독립 최종 review Critical 0; 발견된 Important 요청 경쟁·세션 검증·변화 카드 테스트를 수정하고 pending 실패 경로까지 재검토·보완. supervisor Python 60/60·Vitest 71/71·E2E 10/10·lint·typecheck·build·harness·diff-check 통과; 1440/1024/390 overflow 0·scroll lock·focus 확인 | 4 | 자동·수동 요청 세대 가드, strict ISO 시각, stable row, KST 왕복, hydration, modal focus 적용; 실제 환자정보·영구 DB·새 임상 규칙 비범위 |
