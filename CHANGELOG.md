...
### Fixed — CareNote 차팅 문장 추천

- 짧은 입력을 제공된 사실의 기록 문장으로 추천하고, 채택 후 입력 원문과 비교하는 흐름 복구.
- 빈 O/A/P 자동 생성과 전 항목 작성 강제를 제거. CareNote 합성 데이터 데모에만 별도 부분 SOAP 형식 검증 적용. 원본 앱의 검증 정책은 유지.
- 수기 확인 완료 문구 오차단 및 여러 줄 미완성 표시 누락을 회귀 테스트 후 수정.
- 랜딩 예시·FAQ를 실제 동작과 동기화. 입력 외 관찰·처치·계획 생성, 실제 의료기록 저장, 새 LLM 호출은 추가하지 않음.
- 감독 Vitest 33파일 473건, 브라우저 E2E 86건, Python 155건 및 정적 검사·빌드 통과. 기술 검증이며 사용자 효과 측정이 아님.

## [1.0.2] - 2026-09-23

### Changed — CareNote 제품 소개 리디자인

- 공식 SaaS 레퍼런스 및 Figma 토큰을 바탕으로 중앙 히어로, 큰 제품 미리보기, 기능 설명, FAQ와 체험 동선을 재구성.
- 같은 합성 환자의 근무 준비·원본 근거·간호기록 미리보기를 키보드로 전환. 실제 작업공간 링크 유지.
- 모바일 미리보기 글자·CTA 대비·sticky 헤더·앵커 위치 검증. 첫 대비 회귀는 별도 RED 검사 후 수정.
- 작업공간·임상 로직·API·저장 정책 변경 없음. 가짜 고객·성과·인증·미지원 기능을 소개하지 않음.

## [1.0.1] - 2026-09-23

### Changed — CareNote 문구 정리

- 랜딩·작업공간의 장식 영어, 반복 소개·탭 설명, 정상 완료 배너 삭제.
- 간호기록 중복 제목을 제거하고 근거 빈 화면·입력 도움말을 축약.
- 오류·불완전 원본·원본 시각·읽음·새로고침 소실·미서명 안내와 기존 기능은 유지.
- 전체 Vitest429, 실제 API E2E6, Python155, Next build 통과. 사용성 효과 측정은 아님.

## [1.0.0] - 2026-09-23

### Added — CareNote 별도 제품 데모

- 제품 소개 랜딩 `/`, 공통 환자 맥락의 `/workspace`: 근무 준비·환자 기록·간호기록.
- 5명 합성 시나리오의 명시적 입원 매핑, 원본 근거 패널, 시점 선택, 읽음 및 검토 메모.
- 환자별 메모리 초안·미서명 추가 기록과 deterministic 근무 준비 재조회 연결.
- 지원 단문만 받는 SOAP 초안. 입력 외 사실을 생성하지 않고 미입력 항목은 수기 작성 필요로 남김.
- 전체입력/시간대/미래근거/다른환자/추가실패 경계 및 실 API 통합 E2E.

### Preserved

- 기존 인수인계 비교·기간 변화·SBAR·원본 편집·안내는 `/handover`로 보존.
- 기존 인수인계 및 차팅 Production 서비스, 원본 차팅 저장소는 변경하지 않음.

### Verification

- 감독 최종 Vitest 32파일423건, Python155건, 기존 E2E68건 및 새 실제API E2E6건 통과.
- ESLint, TypeScript, Next production build, harness 통과. 독립 계약/UI 재리뷰에서 미해결 Important 없음.
- 390/960/1440 근거 열기·상태 이동 검증. 자동검사는 사용자 효과나 임상 사용 적합성 입증이 아님.

### Limits

- 새 작업공간의 작성·검토 상태는 페이지 메모리만 사용하며 새로고침 초기화. 공식 저장·서명·병원 연동 없음.
- 새 차팅은 규칙 기반이며 복합/미지원 입력은 수기 작성. V/S 자동 SOAP·PRN 투여 fixture 없음.
- 독립 Vercel 프로젝트 배포 기록은 `docs/verification/2026-09-23-carenote-release.md` 참조.
